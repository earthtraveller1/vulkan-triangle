...
use std::{ffi::CStr, ptr::null_mut, convert::TryInto};
use super::Surface;
...
impl PhysicalDevice {
    ...
    pub fn find_queue_families(&self, surface: &Surface) -> QueueFamilyIndices {
        ...
        unsafe { vkGetPhysicalDeviceQueueFamilyProperties(self.raw_handle, &mut queue_family_count, null_mut()) };
        
        let mut queue_family_properties = Vec::with_capacity(queue_family_count.try_into().unwrap());
        unsafe { 
            vkGetPhysicalDeviceQueueFamilyProperties(self.raw_handle, &mut queue_family_count, queue_family_properties.as_mut_ptr());
        ...
        for i in 0..queue_family_properties.len() {
            ...
            unsafe { vkGetPhysicalDeviceSurfaceSupportKHR(self.raw_handle, i.try_into().unwrap(), surface.raw_handle, &mut present_support) };
